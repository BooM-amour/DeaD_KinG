...
local function run(msg, matches)
  if matches[1]:lower() == 'DeaD' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /DeaD ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "DeaD" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^([Ss]tatslist)$",
    "^([Ss]tats) (group) (%d+)",
    "^([Ss]tats) (DeaD)",-- Put everything you like :)
		"^([Dd]eaD)"-- Put everything you like :)
    }, 
  run = run
}
